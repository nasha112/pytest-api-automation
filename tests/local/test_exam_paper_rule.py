...
@allure.feature("本地Exam系统 - 规则管理")
@allure.story("登录后查询规则列表")
@allure.tag("local", "smoke")
def test_admin_login_and_get_rule_list(base_url):
    ...
    # 查询规则列表
    list_url = f"{base_url}/exam/api/paper/rule/paging"
    payload = {
        "current": 1,
        "size": 10,
        "params": {"title": ""},
    ...
    with allure.step("查询规则列表"):
        ...
        assert len(records) > 0, "规则列表为空"
        titles = [r.get("title") for r in records]
        assert "智商测试题" in str(titles) or "计算机" in str(titles)